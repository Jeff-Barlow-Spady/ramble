...
// This file is a placeholder for when Go bindings are not available.
// The real implementation is in whisper_go_binding_impl.go when the "whisper_go" tag is used.
...
func (t *WhisperGoTranscriber) ProcessAudioChunk(audioData []float32) (string, error) {
	...
	// 2. It's been at least 300ms since last processing
	if len(t.buffer) >= t.minBufferLen && time.Since(t.bufferTime) >= 300*time.Millisecond {
