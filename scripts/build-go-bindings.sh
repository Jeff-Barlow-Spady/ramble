...
CGO_LDFLAGS="-L$(pwd)/whisper.cpp -L$(pwd)/whisper.cpp/build -L$(pwd)/whisper.cpp/build/src -L$(pwd) -L${HOME}/.ramble/lib"
CGO_CFLAGS="-I$(pwd)/whisper.cpp/include -I${HOME}/.ramble/include"
go build -tags=whisper_go -ldflags="-extldflags '-Wl,-rpath,$(pwd) -Wl,-rpath,${HOME}/.ramble/lib'" -o ramble ./cmd/ramble